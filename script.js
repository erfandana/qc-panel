(async function () {
    ...
    if (!panel) {
        try {
            ...
            console.error("Gagal memuat panel UI:", e);
        }
    } else {
    ...
    function closeCamera() {
        if (html5Qrcode) {
            html5Qrcode.stop().catch(() => {}).finally(() => { if (camContainer) camContainer.style.display = "none"; });
        }
    }
    ...
    sizeSelect.addEventListener("change", function () {
        ...
        if (s) {
            capInput.value = s.cap; botolInput.value = s.botol; cartonInput.value = s.carton; toleransiInput.value = s.toleransi.toFixed(2); labelInput.value = s.label; layerInput.value = s.layer; foldingInput.value = s.folding;
        }
        simpanMemoriInput();
        hitungBerat();
    });

    [densityInput, capInput, botolInput, cartonInput, labelInput, layerInput, foldingInput].forEach((i) => i?.addEventListener("input", () => { checkDensity(densityInput); simpanMemoriInput(); hitungBerat(); }));
    ...
    document.getElementById('qc-submit').onclick = async function() {
        ...
        if (inputBatch && divBatch) {
            ...
            divBatch.dispatchEvent(new ClipboardEvent('paste', { bubbles: true, cancelable: true, clipboardData: dataTransfer }));
            divBatch.dispatchEvent(new Event('input', { bubbles: true }));
        }

        // 3. PROSES CEKLIS KEMASAN
        if (sizeSelect.value) {
            const targetVolume = sizeSelect.value.split(' X ')[1]?.toLowerCase().replace(/\s/g, ''); 
            if (targetVolume) {
                document.querySelectorAll('div[class*="checkListItemSimpleChecklist"]').forEach(container => {
                    const labelText = container.querySelector('span').textContent.toLowerCase().replace(/\s/g, '');
                    const input = container.querySelector('input[type="checkbox"]');
                    if (input) {
                        if ((labelText === targetVolume) !== input.checked) input.click();
                    }
                });
            }
        }
        ...
        simpanMemoriInput();
        console.log("Submit berhasil: Semua data disinkronisasi.");
    };
